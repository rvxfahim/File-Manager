...
class sdList{
  public: 
    ...
    static void update_list(){
      ...
      char listofdir[50][30]={0};
      char name_of_file[30] ={0};
      char list_of_file[50][30] ={0};
      int i=0; 
      int j=0;
      ...
      Serial.println("printing list of directories only");
        ...
        */
        ...
        // at this point i and j contains the number of folders and files in the set directory
        //Serial.println("before adding files in list, here is the first file name:");
        //Serial.println(list_of_file[0]);
        
        i=0;
        j=0;
        int listbox_item_pos=0;
        ...
        /*if(listofdir[i][0]=='\0')
        {
          ...
          Serial.println(acTxt);
            if (gslc_ElemXListboxInsertItemAt(&m_gui,m_pElemListbox1,listbox_item_pos,acTxt)) {
              // successfully added new file in the list
              //j=i+1;
              
              Serial.print("added ");
              Serial.print(acTxt);
              Serial.print(" to listbox at position ");
              Serial.println(listbox_item_pos);
              listbox_item_pos++;
              }
              else
              {
...
//<Listbox Enums !Start!>
    case E_ELEM_LISTBOX1:
      if (nSelId != XLISTBOX_SEL_NONE) {
        gslc_ElemXListboxGetItem(&m_gui, pElemRef, nSelId, acTxt2, sizeof(acTxt2));
        setting_done=true;
      }
      else{
        setting_done=false;
